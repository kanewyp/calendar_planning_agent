...
#
#     [group:<id>]                  — phase/topic identifier
...
def safe_structural_shuffle(
    ...
    """Reorder explicitly-shufflable runs within each [group:*] block.

    Safety guarantees:
    - Group order is preserved (groups execute in the order they first appear).
    - Tasks with [seq:N] are hard-locked by sequence — never reordered.
    - Reordering only happens within contiguous runs where ALL tasks have
      [shuffle:yes] AND no sequence id.
    - Within such a run, tasks are sorted by ``run_sort_key`` *descending*;
      callers pass the ordering that best matches their heuristic
