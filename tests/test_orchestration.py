...
class TestGenerateRationales:
    def _base_state(self) -> dict[str, Any]:
        ...
        return {
            ...
            "candidate_deadline_first": list(candidate),
            "candidate_min_fragmentation": list(candidate),
            "candidate_energy_aware": list(candidate),
    ...
    def test_single_strategy_llm_failure_uses_fallback(self):
        state = self._base_state()
        responses = {
            "deadline_first": RuntimeError("provider transient failure"),
            "min_fragmentation": "Fragmentation rationale",
            "energy_aware": "Energy rationale",
        }

        def _fake_call(prompt: str, purpose: str = "rationale") -> str:
            _ = purpose
            for strategy_name, response in responses.items():
                if f"Strategy: {strategy_name}" in prompt:
                    if isinstance(response, Exception):
                        raise response
                    return response
            raise AssertionError("Unexpected prompt strategy")
        ...
        assert "deadline_first" in rationales
        assert "schedules 1 event(s)" in rationales["deadline_first"]
        assert rationales["min_fragmentation"] == "Fragmentation rationale"
        assert rationales["energy_aware"] == "Energy rationale"

        trace = result["debug_trace"][0]
        assert trace["node"] == "generate_rationales"
        assert trace["summary"]["fallback_count"] == 1
        assert trace["details"]["deadline_first"]["source"] == "fallback"
        assert trace["details"]["min_fragmentation"]["source"] == "llm"

    def test_all_strategy_llm_failures_still_return_rationales(self):
        state = self._base_state()
        ...
        rationales = result["candidate_rationales"]
        assert set(rationales.keys()) == {
            "deadline_first",
            "min_fragmentation",
            "energy_aware",
        }
        assert all("preserves the dependency-safe learning flow" in text for text in rationales.values())

        trace = result["debug_trace"][0]
        assert trace["summary"]["fallback_count"] == 3
        assert all(
            info["source"] == "fallback"
            for info in trace["details"].values()
        )
